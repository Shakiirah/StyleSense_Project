...
function StyleQuiz() {
  ...
  const completeQuiz = async () => {
    ...
    const context = {
      ...
      weather: answers.weather || "Mild",
      budgetRange: answers.budget || "$50 - $100",
  ...
  const saveOutfit = () => {
    ...
    if (!getToken()) {
      ...
      navigate("/login", { state: { message: "Sign in to save this outfit to your recommendations." } });
      return;
    }
    ...
    <div className="body-options visual-options">
      {bodyTypes.map((type) => (
        ...
        <div className="quiz-buttons">
         ...
         <button
            className="next-btn"
            onClick={step === 6 ? completeQuiz : nextStep}
            disabled={loading}
          >
          {loading ? "Generating..." : step === 6 ? "Get My Recommendations" : "Next"}
         </button>
        </div>
        ...
        {recommendation && (
          <div className="quiz-result" ref={resultRef}>
            ...
            <p className="result-meta">
              Occasion: {recommendation.occasion || answers.occasion || "Everyday"} · Weather: {answers.weather || "Mild"} · Budget: {answers.budget || "$50 - $100"}
            </p>
            ...
            <div className="result-actions">
              ...
              {!getToken() && (
                <span>Sign in only when you want to save or buy this look.</span>
              )}
            </div>
            ...
            <div className="result-grid">
              {recommendedItems.length > 0 ? (
                recommendedItems.map((item) => (
                  <article className="result-item" key={`${item.category}-${item.id}`}>
                    <div className="result-image">
                      <img src={getProductImage(item)} alt={item.name} onError={(event) => { event.currentTarget.src = fallbackImage; }} />
                    </div>
                ...
                <p className="result-empty">
                  The AI saved a recommendation, but no product items were returned. Try a different occasion or restart the backend with the latest recommendation update.
                </p>
              )}
            </div>
          </div>
